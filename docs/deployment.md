...
Account emails use browser-openable links such as `https://YOUR_API_HOST/auth/verify#token=…`, built from `PUBLIC_URL`. The API serves the landing pages and their embedded assets; no separate web deployment is needed. Keep `ALLOWED_ORIGIN` equal to this HTTPS origin for same-origin browser verification, or use the app handoff if the configured web origin is elsewhere. Route `/auth/*` as well as `/v1/*` through your reverse proxy and do not add third-party scripts or analytics to these token-bearing pages.

The landing page's **Open Holy Hymns** button uses the existing `holyhymns://auth/...` scheme to open the installed app at verification, password reset, or invitation acceptance, with the code filled in. Verification also has an explicit **Verify in browser** action, using the existing rate-limited `POST /v1/auth/verify`; it never creates a session or signs the visitor in. `GET`/`HEAD` previews do not consume tokens. The token stays in the URL fragment (not sent in the initial HTTP request), is removed from browser history after loading, and is submitted only in a POST body or handed to the app after a click. Reopen the original email link after refreshing the page. Responses disable caching, framing, and referrers, and load only local assets. Expired or previously used links keep their existing one-time-token behavior.

This handoff works with the app's existing custom scheme and does not require a new app build. It is not an automatic iOS Universal Link or Android App Link: those additionally require domain-association files, actual signing identifiers/certificate fingerprints, native entitlements, HTTPS link handling, and rebuilt apps. Do not invent these identifiers or enable Associated Domains for a Personal Team build. See [Expo Universal Links](https://docs.expo.dev/linking/ios-universal-links/) and [Android App Links](https://docs.expo.dev/linking/android-app-links/). Existing delivered emails cannot be edited; use their manual code or request a fresh email after deploying the new API.
